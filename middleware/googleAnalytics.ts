...
export const GA4_MEASUREMENT_ID = Deno.env.get("GA4_MEASUREMENT_ID");

export default function createGAMiddleware(server) {
    return async function googleAnalyticsMiddleware(req, next, info) {
        let err;
        let res: Response;
        try {
